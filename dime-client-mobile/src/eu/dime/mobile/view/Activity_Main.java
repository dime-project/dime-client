...
public class Activity_Main extends ActivityDime implements OnClickListener, OnLongClickListener {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        Button peopleButton = (Button) findViewById(R.main.button_people);
        Button myprofileButton = (Button) findViewById(R.main.button_myprofile);
        Button communicationButton = (Button) findViewById(R.main.button_communication);
        Button mydataButton = (Button) findViewById(R.main.button_mydata);
        Button settingsButton = (Button) findViewById(R.main.button_settings);
        ImageButton logout = (ImageButton) findViewById(R.main.logout);
        Button calendarButton = (Button) findViewById(R.main.button_calendar);
        ...
        mydataButton.setOnClickListener(this);
    ...
    protected void onResume() {
        ...
        dimeUser.setText(DimeClient.getUserMainSaid() + "@dime");
        HTTPResponse result = new DimeHelper().dimeServerIsAlive(DimeClient.getUserMainSaid(), AndroidModelHelper.getModelConfiguration().restApiConfiguration);
        if(result != null && !(result.code == 200)){
        	Toast.makeText(getApplicationContext(), "", Toast.LENGTH_SHORT).show();
        } else {
			dimeVersion.setText(eu.dime.mobile.helper.AndroidModelHelper.getVersion(result));
        	startTask("");
        }
    }
    ...
    protected void loadData() {
        situations = (List<SituationItem>) (Object) Model.getInstance().getAllItems(mrContext, TYPES.SITUATION);
        notifications = (List<UserNotificationItem>) (Object) Model.getInstance().getAllItems(mrContext, TYPES.USERNOTIFICATION);
        isPlaceAdapterConnected = ModelHelper.isPlaceAdapterConnected(mrContext);
        if(isPlaceAdapterConnected) {
	        Place placeTmp = ContextHelper.getCurrentPlace();
	        if (placeTmp != null) {
	            currentPlace = (PlaceItem) Model.getInstance().getItem(mrContext, TYPES.PLACE, placeTmp.getPlaceId());
	        }
        }
    }
