...
public class UIHelper {
	...
	public static void createStandardDialog(final Activity activity, ModelRequestContext mrContext, final BaseAdapterDisplayableItem adapter, List<DisplayableItem> items, final ResultObject.RESULT_OBJECT_TYPES type) {
		try {
			...
			TextView emptyText = new TextView(activity.getApplicationContext());
			...
			adapter.init(activity, mrContext, list, items);
			...
			builder.setPositiveButton("confirm", new DialogInterface.OnClickListener() {
				...
				@Override
				public void onClick(DialogInterface dialog, int which) {
					if (activity instanceof ResultsOfStandardDialogInterface) {
						ResultsOfStandardDialogInterface rosdi = (ResultsOfStandardDialogInterface) activity;
						if (adapter instanceof BaseAdapter_Dialog_Sharing_Profile) {
							ProfileItem item = ((BaseAdapter_Dialog_Sharing_Profile) adapter).getSelectedProfile();
						...
						} else if (adapter instanceof BaseAdapter_Livepost) {
							List<LivePostItem> liveposts = (List<LivePostItem>) (Object) adapter.getSelectionItems();
							rosdi.handleResult(new ResultObjectLiveposts(liveposts));
						} else if (adapter instanceof BaseAdapter_ServiceAdapter) {
							ServiceAdapterItem item = ((BaseAdapter_ServiceAdapter) adapter).getSelectedServiceAdapter();
	...
	public static void showItemActionDialog(Context context, DimeIntentObject dio) {
		...
		ItemActionListener listener = new ItemActionListener(context, dio);
		Dialog dialog = createActionDialog(context, Arrays.asList(actions), listener, Arrays.asList(dio.getItemId()));
		listener.setDialog(dialog);
		dialog.show();
		if(!dio.getOwnerId().equals(Model.ME_OWNER)) {
	...
	public static void inflateStandardHeader(final Activity activity, DisplayableItem di, ModelRequestContext mrContext) {
		if (di != null) {
	...
	public static View createWarningWidget(Context context, AdvisoryItem advisoryItem) {
		LinearLayout ll = new LinearLayout(context);
		ImageView image = new ImageView(context);
		image.setLayoutParams(new LayoutParams(40, 40));
		image.setPadding(0, 2, 10, 0);
	...
	public static String getTextForWarning(Context context, AdvisoryItem advisoryItem) {
		...
		String message = (advisoryItem.getWarningLevel() <= 0.5) ? resources.getString(R.string.sharing_warningLevel_low) : resources.getString(R.string.sharing_warningLevel_high);
		// untrusted
		if (advisoryItem.getWarningType().equals(AdvisoryItem.WARNING_TYPES[0])) {
			...
			message += " " 
					+ formatStringList(AndroidModelHelper.getListOfNamesOfGuidList(context, attributes.getPrivateResources(), true)) + " with "
					+ getWarningText(AndroidModelHelper.getTrustOrPrivacyLevelForDisplayableItem(attributes.getPrivacyValue())) + 
					" privacy level to " 
					+ formatStringList(AndroidModelHelper.getListOfNamesOfGuidList(context, attributes.getUntrustedAgents(), false)) + " with "
					+ getWarningText(AndroidModelHelper.getTrustOrPrivacyLevelForDisplayableItem(attributes.getTrustValue())) +
					" trust level!";
		}
		// disjunct_groups
			...
			message += " " + attributes.getConcernedResources() 
					+ " to receiver " 
					+ formatStringList(AndroidModelHelper.getListOfNamesOfGuidList(context, attributes.getConcernedPersons(), false))
		...
		// profile_not_shared
			...
			message += " via the selected profile because "
		...
		// too_many_resources
			...
			message += " more than " 
					...
					+ " resources to selected receivers at once!";
		}
		// too_many_receivers
			...
			message += " the selected resources to more than " 
